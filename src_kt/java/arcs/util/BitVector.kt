...
class BitVector(
   ...
   *
   * @param bitRange range of bits to select from the [byte], where for each value `i` in the range:
   *   `byte\[i]` is the `i`th bit from the right. E.g. `appendBits(0b01110000, 4..6)` would select
   *   the 1s.
   */
  fun appendBits(byte: Byte, bitRange: IntRange = 0..7) {
    val numBits = bitRange.last - bitRange.first + 1

    require(bitRange.first >= 0 && bitRange.last <= 7) { "bitRange must be a subrange of [0, 8)" }
  ...
  fun asSequenceOfInts(bitsPerValue: Int): Sequence<Window> {
    require(bitsPerValue in 1..32) {
      "Cannot return sequence of Ints where each Int has more bits than are allowed in an Int (32)."
    }
