...
class StoreTest {
  ...
  fun doesntSendUpdateToDriver_afterDriverOriginatedMessages() = runBlockingTest {
    ...
    // Note that this assumes no asynchrony inside store.ts. This is guarded by the following test,
    // which will fail if driver.receiver() doesn't synchronously invoke driver.send().
