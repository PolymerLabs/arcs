  ...
  fun threeBitValues() {
    ...
    BitVector(alternatingBytes)
      .asSequenceOfInts(3).forEachIndexed { index, window ->
        ...
        assertWithMessage("${window.value.toString(2)} should be ${expectedValue.toString(2)}")
          .that(window.value).isEqualTo(expectedValue)
        if (index == 32 / 3) {
          assertWithMessage("Last should have 1 padding bit").that(window.paddingBits).isEqualTo(1)
        } else {
          assertWithMessage("No padding bits").that(window.paddingBits).isEqualTo(0)
        }
      }
    ...
    BitVector(alternatingBytes)
      .asSequenceOfInts(6).forEachIndexed { index, window ->
        if (index == 5) {
          ...
          assertWithMessage("Should have four padding bits").that(window.paddingBits).isEqualTo(4)
        } else {
