...
<div master>
  ...
  <template selectable>
    <div selectable-item on-click="onSelect" key="{{index}}" trigger$="{{name}}">
    ...
    <div slotid="annotation" subid$="{{id}}" style="padding:4px 0;"></div>
  </template>
  ...
  return class extends DomParticle {
    ...
    render({list}) {
      const items = list.map(({rawData}, i) => {
    ...
    onSelect(e) {
      this.handles.get('selected').set(this.props.list[e.data.key]);
    }
  };
