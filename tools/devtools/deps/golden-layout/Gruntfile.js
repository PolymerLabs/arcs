/// BareSpecifier=golden-layout\Gruntfile
...
module.exports = function (grunt) {
	...
	var sources = ['./build/ns.js', './src/js/utils/utils.js', './src/js/utils/EventEmitter.js', './src/js/utils/DragListener.js', './src/js/**'];

	var basicGulpStream = function (stream) {
		return stream.pipe(concat('goldenlayout.js')).pipe(insert.wrap('(function($){', '})(window.$);'));
	};
			...
			glmin: {
				options: {
					tasks: function (stream) {
						return basicGulpStream(stream).pipe(uglify()).pipe(concat('goldenlayout.min.js'));
					}
				},
				src: sources,
