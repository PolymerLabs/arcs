/// BareSpecifier=golden-layout\start
$(function () {
	...
	myLayout.registerComponent('html', function (container, state) {
		container.getElement().html(state.html ? state.html.join('\n') : '<p>' + container._config.title + '</p>');
		...
		if (state.bg) {
			container.getElement().text('hey').append('<br/>').append($('<button>').on('click', function () {
				rotate(container);
			}).text('rotate header')).append('<br/>').append($('<button>').on('click', nexttheme).text('next theme'));
		}
	});
			...
			content: [{
				...
				content: [{
					...
					componentState: {
						...
						style: ['.market-content label {', '  margin-top: 10px;', '  display: block;', '  text-align: left;', '}', '.market-content input {', '  width: 250px;', '  border: 1px solid red', '}'],
