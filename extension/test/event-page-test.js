...
describe('EventPage', () => {
  describe('#_prepareResults()', () => {
    it('should prepare keys', () => {
      ...
      assert.isOk(result);
      assert.isOk(result.hasOwnProperty('urla'),
    ...
    it('should trim out keys without values', () => {
      ...
      assert.isOk(result.hasOwnProperty('urla'),
        `result didn't have a urla key ${result}`);
      assert.isNotOk(result.hasOwnProperty('urlb'),
        `result had an 'empty' key (presumably with an empty result list: ${result['empty']})`);
    });
  });
});
