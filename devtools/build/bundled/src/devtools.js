...
(() => {
  let msgQueue = [];
  let windowForEvents = undefined; // Use the extension API if we're in devtools and having a window to inspect.
  ...
  let sendMessage = chrome.devtools && chrome.devtools.inspectedWindow.tabId ? connectViaExtensionApi() : connectViaWebSocket();

  if (chrome && chrome.devtools) {
    // Add the panel for devtools, and flush the events to it once it's shown.
    chrome.devtools.panels.create('Arcs', null, '../build/split-index.html', panel => panel.onShown.addListener(panelWindow => initializeWindow(panelWindow)));
  } else {
  ...
  function connectViaExtensionApi() {
    ...
    backgroundPageConnection.onMessage.addListener(e => queueOrFire(e));
    return msg => {
      backgroundPageConnection.postMessage({
        name: 'command',
        tabId: chrome.devtools.inspectedWindow.tabId,
  ...
  function fire(msg) {
    windowForEvents.document.dispatchEvent(new CustomEvent('messages', {
      detail: msg
    }));
  }
})();