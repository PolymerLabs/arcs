...
describe('FlowGraph validation', () => {
  it('SLANDLES SYNTAX succeeds when there are no checks', async () => {
  ...
  it('SLANDLES SYNTAX succeeds when a check is satisfied directly', async () => {
    const graph = await buildFlowGraph(`
      particle P1
        foo: writes Foo {}
        claim foo is trusted
  ...
  it('SLANDLES SYNTAX fails when the edge has no ingress', async () => {
    const graph = await buildFlowGraph(`
      particle P1
        foo: writes Foo {}
        claim foo is trusted
  ...
  it('SLANDLES SYNTAX fails when a different tag is claimed', async () => {
    const graph = await buildFlowGraph(`
      particle P1
        foo: writes Foo {}
  ...
  it('SLANDLES SYNTAX fails when a "not tag" is claimed and the tag is checked for', async () => {
    const graph = await buildFlowGraph(`
      particle P1
        foo: writes Foo {}
  ...
  it('SLANDLES SYNTAX succeeds when a "not tag" is claimed and there are no checks', async () => {
    const graph = await buildFlowGraph(`
      particle P1
        foo: writes Foo {}
        claim foo is not trusted
  ...
  it('SLANDLES SYNTAX fails when a "not tag" cancels a tag', async () => {
    const graph = await buildFlowGraph(`
      particle P1
        foo: writes Foo {}
        claim foo is trusted
  ...
  it('SLANDLES SYNTAX succeeds when a "not tag" cancels a tag that is reclaimed downstream', async () => {
    const graph = await buildFlowGraph(`
      particle P1
        foo: writes Foo {}
        claim foo is trusted
  ...
  it('SLANDLES SYNTAX succeeds for a negated tag check when the tag is missing', async () => {
    const graph = await buildFlowGraph(`
      particle P
        bar: reads Foo {}
  ...
  it('SLANDLES SYNTAX fails for a negated tag check when the tag is present', async () => {
    const graph = await buildFlowGraph(`
      particle P1
        foo: writes Foo {}
  ...
  it('SLANDLES SYNTAX succeeds when an inout handle claims the same tag it checks', async () => {
    const graph = await buildFlowGraph(`
      particle P
  ...
  it('SLANDLES SYNTAX fails when an inout handle claims a different tag from the one it checks', async () => {
    const graph = await buildFlowGraph(`
      particle P
        foo: reads writes Foo {}
        check foo is t1
  ...
  it('SLANDLES SYNTAX succeeds when handle has multiple inputs with the right tags', async () => {
    const graph = await buildFlowGraph(`
      particle P1
        foo: writes Foo {}
        claim foo is trusted
  ...
  it('SLANDLES SYNTAX fails when handle has multiple inputs but one is untagged', async () => {
    const graph = await buildFlowGraph(`
      particle P1
        foo: writes Foo {}
        claim foo is trusted
  ...
  it('SLANDLES SYNTAX fails when handle has no inputs', async () => {
    const graph = await buildFlowGraph(`
      particle P
        bar: reads Foo {}
        check bar is trusted
  ...
  it('SLANDLES SYNTAX claim propagates through a chain of particles',  async () => {
    const graph = await buildFlowGraph(`
      particle P1
        foo: writes Foo {}
        claim foo is trusted
  ...
  it('SLANDLES SYNTAX a claim made later in a chain of particles does not override claims made earlier', async () => {
    const graph = await buildFlowGraph(`
      particle P1
        foo: writes Foo {}
        claim foo is trusted
  ...
  it('SLANDLES SYNTAX succeeds when a check includes multiple tags', async () => {
    const graph = await buildFlowGraph(`
      particle P1
        foo: writes Foo {}
        claim foo is tag1
  ...
  it(`SLANDLES SYNTAX fails when a check including multiple tags isn't met`, async () => {
    const graph = await buildFlowGraph(`
      particle P1
        foo: writes Foo {}
        claim foo is tag1
  ...
  it(`SLANDLES SYNTAX succeeds when a check including multiple anded tags is met by a single claim`, async () => {
    const graph = await buildFlowGraph(`
      particle P1
        foo: writes Foo {}
  ...
  it(`SLANDLES SYNTAX succeeds when a check including multiple 'or'd tags is met by a single claim`, async () => {
    const graph = await buildFlowGraph(`
      particle P1
        foo: writes Foo {}
        claim foo is tag1 and is tag2
  ...
  it('SLANDLES SYNTAX can detect more than one failure for the same check', async () => {
    const graph = await buildFlowGraph(`
      particle P1
        foo: writes Foo {}
        claim foo is notTrusted
  ...
  it('SLANDLES SYNTAX can detect failures for different checks', async () => {
    const graph = await buildFlowGraph(`
      particle P1
        foo1: writes Foo {}
        foo2: writes Foo {}
  ...
  it('SLANDLES SYNTAX supports datastore tag claims', async () => {
    const graph = await buildFlowGraph(`
      schema MyEntity
        text: Text
      resource MyResource
  ...
  describe(`'is from handle' check conditions`, () => {
    it('SLANDLES SYNTAX succeeds when the handle is exactly the same', async () => {
      const graph = await buildFlowGraph(`
        particle P
          input1: reads Foo {}
          input2: reads Foo {}
    ...
    it('SLANDLES SYNTAX fails when handle is different', async () => {
      const graph = await buildFlowGraph(`
        particle P
          input1: reads Foo {}
          input2: reads Foo {}
    ...
    it('SLANDLES SYNTAX succeeds for a negated handle check when the handle is different', async () => {
      const graph = await buildFlowGraph(`
        particle P
          input1: reads Foo {}
          input2: reads Foo {}
    ...
    it('SLANDLES SYNTAX fails for a negated handle check when the handle is the same', async () => {
      const graph = await buildFlowGraph(`
        particle P
          input1: reads Foo {}
          input2: reads Foo {}
    ...
    it('SLANDLES SYNTAX succeeds on an inout handle checking against itself', async () => {
      const graph = await buildFlowGraph(`
        particle P
          foo: reads writes Foo {}
    ...
    it('SLANDLES SYNTAX succeeds when the handle has inputs', async () => {
      const graph = await buildFlowGraph(`
        particle P1
          output1: writes Foo {}
          output2: writes Foo {}
    ...
    it('SLANDLES SYNTAX succeeds when the handle is separated by a chain of other particles', async () => {
      const graph = await buildFlowGraph(`
        particle P1
          input: reads Foo {}
          output: writes Foo {}
    ...
    it('SLANDLES SYNTAX succeeds when the handle is separated by another particle with a claim', async () => {
      const graph = await buildFlowGraph(`
        particle P1
          input: reads Foo {}
          output: writes Foo {}
    ...
    it('SLANDLES SYNTAX fails when another handle is also found', async () => {
      const graph = await buildFlowGraph(`
        particle P1
          input1: reads Foo {}
          input2: reads Foo {}
  ...
  describe(`'is from output' check conditions`, () => {
    it('SLANDLES SYNTAX succeeds when the output is directly connected to the input', async () => {
      const graph = await buildFlowGraph(`
        particle P
          foo: reads Foo {}
          bar: writes Foo {}
    ...
    it('SLANDLES SYNTAX fails when the output is directly connected to an ingress input', async () => {
      const graph = await buildFlowGraph(`
        particle P
          foo: reads Foo {}
          bar: writes Foo {}
    ...
    it('SLANDLES SYNTAX succeeds when the output is separated from the input by another particle', async () => {
      const graph = await buildFlowGraph(`
      particle P1
        foo: reads Foo {}
        bar: writes Foo {}
    ...
    it('SLANDLES SYNTAX fails when another particle writes to the same handle', async () => {
      const graph = await buildFlowGraph(`
      particle P1
        foo: reads Foo {}
        bar: writes Foo {}
  ...
  describe(`'is from store' check conditions`, () => {
    it('SLANDLES SYNTAX succeeds when the data store identified by name is present', async () => {
      const graph = await buildFlowGraph(`
        schema MyEntity
          text: Text
        resource MyResource
    ...
    it('SLANDLES SYNTAX succeeds when the data store identified by ID is present', async () => {
      const graph = await buildFlowGraph(`
        schema MyEntity
          text: Text
        resource MyResource
    ...
    it('SLANDLES SYNTAX succeeds for a negated store check when the store is different', async () => {
      const graph = await buildFlowGraph(`
        schema MyEntity
          text: Text
        resource MyResource
    ...
    it('SLANDLES SYNTAX fails for a negated store check when the store is the same', async () => {
      const graph = await buildFlowGraph(`
        schema MyEntity
          text: Text
        resource MyResource
    ...
    it('SLANDLES SYNTAX fails when the data store identified by name is missing', async () => {
    ...
    it('SLANDLES SYNTAX fails when the data store identified by ID is missing', async () => {
      await assertThrowsAsync(async () => await buildFlowGraph(`
        particle P
          input: reads Foo {}
          check input is from store 'my-store-id'
    ...
    it('SLANDLES SYNTAX fails when the data store is not connected', async () => {
      await assertThrowsAsync(async () => await buildFlowGraph(`
        schema MyEntity
          text: Text
        resource MyResource
    ...
    it('SLANDLES SYNTAX fails when the wrong data store is present', async () => {
      const graph = await buildFlowGraph(`
        schema MyEntity
          text: Text
        resource MyResource
  ...
  describe(`checks using the 'or' operator`, () => {
    it('SLANDLES SYNTAX succeeds when only the handle is present', async () => {
      const graph = await buildFlowGraph(`
        particle P1
          output: writes Foo {}
        particle P2
    ...
    it('SLANDLES SYNTAX succeeds when only the tag is present', async () => {
      const graph = await buildFlowGraph(`
        particle P1
          output: writes Foo {}
    ...
    it('SLANDLES SYNTAX fails when neither condition is present', async () => {
      const graph = await buildFlowGraph(`
        particle P1
          output: writes Foo {}
        particle P2
  ...
  describe(`checks using the 'and' operator`, () => {
    it('SLANDLES SYNTAX succeeds when both conditions are met', async () => {
      const graph = await buildFlowGraph(`
        particle P1
          output: writes Foo {}
          claim output is trusted
    ...
    it('SLANDLES SYNTAX fails when only one condition is met', async () => {
      const graph = await buildFlowGraph(`
        particle P1
          output: writes Foo {}
    ...
    it(`SLANDLES SYNTAX handles nesting of boolean conditions`, async () => {
  ...
  describe('cycles', () => {
    it('SLANDLES SYNTAX supports tag checks in a single-particle cycle', async () => {
      const graph = await buildFlowGraph(`
        particle P
          input: reads Foo {}
          output: writes Foo {}
    ...
    it('SLANDLES SYNTAX supports handle checks tags in a single-particle cycle', async () => {
      const graph = await buildFlowGraph(`
        particle P
          input: reads Foo {}
          output: writes Foo {}
    ...
    it('SLANDLES SYNTAX works with simple two-particle cycles', async () => {
      const graph = await buildFlowGraph(`
        particle P1
          input: reads Foo {}
          output: writes Foo {}
    ...
    it(`SLANDLES SYNTAX supports breaking cycles using 'derives from' claims`, async () => {
    ...
    it('SLANDLES SYNTAX tags propagate throughout overlapping cycles', async () => {
    ...
    it(`SLANDLES SYNTAX a simple cycle doesn't prevent claims from propagating`, async () => {
    ...
    it('SLANDLES SYNTAX two origin cycles can co-exist happily', async () => {
    ...
    it('SLANDLES SYNTAX tags can be removed in a simple cycle ', async () => {
      const graph = await buildFlowGraph(`
        particle P1
          input: reads Foo {}
          output: writes Foo {}
    ...
    it('SLANDLES SYNTAX tags can be removed by a cycle along a chain', async () => {
    ...
    it('SLANDLES SYNTAX overlapping cycles with no ingress fail', async () => {
      const graph = await buildFlowGraph(`
        particle P
          input1: reads Foo {}
          input2: reads Foo {}
  ...
  describe('references', () => {
    it('SLANDLES SYNTAX prunes unrelated inputs', async () => {
      const graph = await buildFlowGraph(`
        particle P1
          foo: writes [Foo {}]
          bar: writes [Bar {}]
    ...
    it('SLANDLES SYNTAX "derives from" claims override reference pruning', async () => {
      const graph = await buildFlowGraph(`
        particle P1
          foo: writes [Foo {}]
          bar: writes [Bar {}]
