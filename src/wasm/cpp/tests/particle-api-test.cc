...
public:
  ...
  void onHandleUpdate(const std::string& name) override {
    Test_data out;
    if (auto input = getSingleton<Test_data>(name)) {
  ...
  arcs::Singleton<Test_data> input1_;
  arcs::Singleton<Test_data> input2_;
  arcs::Collection<Test_data> output_;
};
...
public:
  ...
  void onHandleUpdate(const std::string& name) override {
    const Test_renderFlags& flags = flags_.get();
    renderSlot("root", flags._template(), flags.model());
  }

  arcs::Singleton<Test_renderFlags> flags_;
};
...
public:
  ...
  std::string getTemplate(const std::string& slot_name) override {
    const Test_data& data = data_.get();
    return data.has_txt() ? data.txt() : "empty";
  }

  arcs::Singleton<Test_data> data_;
};
...
public:
  ...
  arcs::Singleton<Test_data> output_;
};
...
public:
  ...
  arcs::Collection<Test_serviceResponse> output_;
};
...
public:
  ...
  void report(const std::string& label, const arcs::Ref<Test_data>& ref) {
    Test_data d;
  ...
  arcs::Singleton<arcs::Ref<Test_data>> sng_;
  arcs::Collection<arcs::Ref<Test_data>> col_;
  arcs::Collection<Test_data> res_;
};
