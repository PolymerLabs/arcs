...
public:
  ...
  void fireEvent(const std::string& slot_name, const std::string& handler) override {
    if (handler == "case1") {
      ...
      arcs::Data d = arcs::clone_entity(in_.get());
      d.set_num(d.num() * 2);
      out_.set(&d);
    } else if (handler == "case3") {
      arcs::Data d = arcs::clone_entity(io_.get());
      d.set_num(d.num() * 3);
      io_.set(&d);
    }
  }

  arcs::Singleton<arcs::Data> in_;
  arcs::Singleton<arcs::Data> out_;
  arcs::Singleton<arcs::Data> io_;
};
...
public:
  ...
  arcs::Collection<arcs::Data> in_;
  arcs::Collection<arcs::Data> out_;
  arcs::Collection<arcs::Data> io_;
  arcs::Data stored_;
};
