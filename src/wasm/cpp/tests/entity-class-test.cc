...
static auto converter() {
  return [](const Test_data& d) { return arcs::entity_to_str(d); };
}
...
public:
  ...
  void test_entity_equality() {
    ...
    for (Test_data* d : std::vector{&d1, &d2}) {
      d->set_num(3);
      d->set_txt("abc");
      d->set_lnk("");
      d->set_flg(false);
  ...
  void test_clone_entity() {
    Test_data src;
    Test_data d1 = arcs::clone_entity(src);
    EQUAL(d1, src);
    ...
    src.set_txt("def");
    src.set_flg(false);
    Test_data d2 = arcs::clone_entity(src);
    EQUAL(d2, src);
    ...
    Test_data d3 = arcs::clone_entity(src);
  ...
  void test_stl_set() {
    ...
    // duplicate
    Test_data d5 = arcs::clone_entity(d4);
...
public:
  ...
  void test_general_usage() {
    ...
    Test_specialFields s2 = arcs::clone_entity(s1);
    NOT_EQUAL(s1, s2);
    NOT_LESS(s1, s2);
    LESS(s2, s1);
