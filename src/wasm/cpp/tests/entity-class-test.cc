...
static auto converter() {
  return [](const arcs::Dummy_data& d) { return arcs::entity_to_str(d); };
}
...
public:
  ...
  void test_entity_equality() {
    ...
    for (arcs::Dummy_data* d : std::vector{&d1, &d2}) {
      d->set_num(3);
      d->set_txt("abc");
      d->set_lnk("");
      d->set_flg(false);
  ...
  void test_clone_entity() {
    arcs::Dummy_data src;
    arcs::Dummy_data d1 = arcs::clone_entity(src);
    EQUAL(d1, src);
    ...
    src.set_txt("def");
    src.set_flg(false);
    arcs::Dummy_data d2 = arcs::clone_entity(src);
    EQUAL(d2, src);
    ...
    arcs::Dummy_data d3 = arcs::clone_entity(src);
  ...
  void test_stl_vector() {
    arcs::Dummy_data d1, d2, d3;
    d1.set_num(12);
    d2.set_num(12);
    Accessor::set_id(&d3, "id");

    std::vector<arcs::Dummy_data> v;
  ...
  void test_stl_set() {
    ...
    // duplicate
    arcs::Dummy_data d5 = arcs::clone_entity(d4);
    Accessor::set_id(&d5, "id");

    std::set<arcs::Dummy_data> s;
  ...
  void test_stl_unordered_set() {
    ...
    std::unordered_set<arcs::Dummy_data> s;
    s.insert(std::move(d1));
    s.insert(std::move(d2));
    s.insert(std::move(d3));
    s.insert(std::move(d4));
...
public:
  ...
  void test_language_keyword_field() {
    arcs::arcs::Dummy_special_fields s;
  ...
  void test_general_usage() {
    arcs::arcs::Dummy_special_fields s1;
    ...
    arcs::arcs::Dummy_special_fields s2 = arcs::clone_entity(s1);
    NOT_EQUAL(s1, s2);
    NOT_LESS(s1, s2);
    LESS(s2, s1);
