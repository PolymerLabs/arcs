...
export class PlanningTestHelper extends TestHelper {
  ...
  async acceptSuggestion(options?: {hostedParticles?: string[], particles?: string[], descriptionText?: string}): Promise<void> {
    ...
    this.log(`Accepting suggestion: '${suggestion.descriptionText ? ((str) => str.length > 50 ? str.substring(0, Math.min(str.length, 50)).concat('...') : str)(suggestion.descriptionText) : suggestion.descriptionText}'`);
