    ...
    switch (op.type) {
      case EntityOpTypes.Set:
        return this.model.singletons[op.field].applyOperation(
          {type: SingletonOpTypes.Set, actor: op.actor, value: op.value, clock: op.clock}
        );
      case EntityOpTypes.Clear:
        return this.model.singletons[op.field].applyOperation(
          {type: SingletonOpTypes.Clear, actor: op.actor, clock: op.clock}
        );
      case EntityOpTypes.Add:
        return this.model.collections[op.field].applyOperation(
          {type: CollectionOpTypes.Add, actor: op.actor, clock: op.clock, added: op.value}
        );
      case EntityOpTypes.Remove:
        return this.model.collections[op.field].applyOperation(
          {type: CollectionOpTypes.Remove, actor: op.actor, clock: op.clock, removed: op.value}
        );
      default:
