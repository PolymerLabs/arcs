...
function RebootInitializer(target: {}, propertyKey: string, parameterIndex: number) {
  set(target.constructor, propertyKey, parameterIndex, {type: MappingType.Direct, initializer: true, reboot: true});
}
      ...
      for (const f of functions.keys()) {
        ...
        const impl = async function(this: APIPort, ...args) {
          ...
          if (initializer !== -1) {
            ...
            } else if (descriptor[initializer].reboot) {
...
export abstract class PECOuterPort extends APIPort {
  ...
  RebootParticle(@RebootInitializer particle: recipeParticle.Particle, @Identifier @Direct id: string) {}
