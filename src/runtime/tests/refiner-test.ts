...
describe('refiner', () => {
    ...
    it('tests complement BigIntRange', Flags.withFieldRefinementsAllowed(async () => {
      ...
      console.log('ten', singleValue['segments']);
      const not10 = singleValue.complement();
      console.log('!=ten', not10['segments']);
      const not_not10 = not10.complement();
      console.log('==ten', not_not10['segments']);
  ...
  describe('NumberRange', () => {
      ...
      it('tests intersection operations on a range', () => {
          ...
          // range1 = (5, 10] U [20, 25);
          assert.deepEqual(range1, new NumberRange([NumberSegment.openClosed(5, 10), NumberSegment.closedOpen(20, 25)]));
          range1.intersectWithSeg(NumberSegment.closedOpen(5, 15));
          // range1 = (5, 10];
          console.log((range1 as any).segments);
          assert.deepEqual(range1, new NumberRange([NumberSegment.openClosed(5, 10)]));
          ...
          // range1 = (5, 10);
          assert.deepEqual(range1, new NumberRange([NumberSegment.openOpen(5, 10)]));
      });
      ...
      it('tests the difference of ranges', () => {
          ...
          assert.deepEqual(diff, new NumberRange([NumberSegment.openOpen(Number.NEGATIVE_INFINITY, 0), NumberSegment.openOpen(10, 20), NumberSegment.openOpen(30, Number.POSITIVE_INFINITY)]));
  ...
  describe('BigIntRange', () => {
      ...
      it('tests union operations on a range', () => {
          ...
          // range1 = [0, 10] U (20,30];
          console.log('HERE1', (range1 as any).segments);
          ...
          // range1 = [0, 15) U (20,30];
          console.log('HERE2', (range1 as any).segments);
          ...
          range1.union(range2);
          // range1 = [-1, -1] U [0, 15) U (15, 19] U (20, 30]
          console.log('HERE3', (range1 as any).segments);
  ...
  describe('BigIntFractions', () => {
    ...
    it('tests fraction addition works 1', () => {
      ...
      let frac1 = new BigIntFraction(num1, den1); // (a^2+a+9)/2a
      let num2 = new BigIntMultinomial({
        [a.toKey()]: BigInt(1),                     // a
        [cnst.toKey()]: BigInt(5)                   // 5
      }); // a+5
      let den2 = new BigIntMultinomial({
        [cnst.toKey()]: BigInt(3)                   // 3
      }); // 3
      let frac2 = new BigIntFraction(num2, den2); // (a+5)/3
      let sum = BigIntFraction.add(frac1, frac2); // (5a^2+13a+27)/6a
      assert.deepEqual(sum.num.terms, {
        [a2.toKey()]: BigInt(5),                    // a^2
        [a.toKey()]: BigInt(13),                    // a
        [cnst.toKey()]: BigInt(27)                  // 9
      });
      assert.deepEqual(sum.den.terms, {
        [a.toKey()]: BigInt(6)                      // a
      });
      ...
      frac1 = new BigIntFraction(num1);           // a/1
      num2 = new BigIntMultinomial({
        [cnst.toKey()]: BigInt(5)                   // 5
      }); // 5
      den2 = new BigIntMultinomial({
        [cnst.toKey()]: BigInt(9)                   // 9
      }); // 9
      frac2 = new BigIntFraction(num2, den2);     // 5/9
      sum = BigIntFraction.add(frac1, frac2);     // (9a+5)/9
      assert.deepEqual(sum.num.terms, {
        [a.toKey()]: BigInt(9),                     // 9a
        [cnst.toKey()]: BigInt(5)                   // 5
      });
