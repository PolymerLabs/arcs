...
annotation allowedRetention(medium: Text, encryption: Boolean)
  ...
  retention: Source
  allowMultiple: true
  doc: 'Indicates the conditions under which the target data can be retained. Valid values for the medium are: Ram, Disk.'

annotation maxAge(age: Text)
  targets: [PolicyTarget]
  retention: Source
  doc: 'Indicates the maximum age the target data can be in order to be used. Age is a string like "2days".'
...
annotation allowedUsage(label: Text, usageType: Text)
  ...
  retention: Source
  allowMultiple: true
  doc: 'Indicates the conditions under which a field can be used. Valid values for usageType are: egress, join, *.'
`;
