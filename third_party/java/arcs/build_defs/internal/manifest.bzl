...
def _generate_root_manifest_content(label, input_files):
    ...
    num_folders_up = label.package.count("/") + 1 if label.package else 0
    relative_path = "../" * num_folders_up

    content = ["// Root manifest generated by %s." % label]
    for input_file in input_files:
        if input_file.basename.endswith(".arcs"):
            content.append("import '%s'" % (relative_path + input_file.short_path))
    return "\n".join(content)
...
def _arcs_manifest_bundle(ctx):
    ...
    root_manifest_filename = ctx.label.name + ".root.arcs"
    ...
    input_files.append(root_manifest_file)
