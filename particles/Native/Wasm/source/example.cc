...
public:
  ...
  void populateModel(const std::string& slot_name, arcs::Dictionary* model) override {
    const BasicParticle_foo& product = foo_.get();
  ...
  void fireEvent(const std::string& slot_name, const std::string& handler) override {
    if (handler == "clicky") {
      BasicParticle_foo copy = arcs::clone_entity(foo_.get());  // does not copy internal entity id
...
private:
  arcs::Singleton<BasicParticle_foo> foo_;
  arcs::Collection<BasicParticle_bar> bar_;
  int num_clicks_ = 0;
};
...
private:
  arcs::Collection<Watcher_bar> bar_;
};
