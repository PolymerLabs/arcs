...
export const Pipe = {
  ...
  async receiveEntity(context, recipes, callback, deviceClient, json) {
    //log('receiveEntity', json);
    t0 = now();
    const type = extractType(json);
    const recipe = recipeForType(type, recipes);
    if (recipe) {
      return instantiateAutofillArc(context, type, recipe, callback, deviceClient);
    } else {
...
const instantiateAutofillArc = async (context, type, recipe, callback, deviceClient) => {
  const arc = await Utils.spawn({id: 'piping-arc', composer: new RamSlotComposer(), context, deviceClient});
  log(`arc [${arc.id}]`);
