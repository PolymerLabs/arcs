...
export class HandleConnection {
  ...
  toPrettyString() { return `${this.particle.name}::${this.name}`; }
  ...
  _isValid(options) {
    if (this.direction && !['in', 'out', 'inout', 'host'].includes(this.direction)) {
      if (options && options.errors) {
        options.errors.set(this, `Invalid direction '${this.direction}' for handle connection '${this.toPrettyString()}'`);
      }
      return false;
    }
    if (this.type && this.spec) {
      ...
      if (!connectionSpec.isCompatibleType(this.rawType)) {
        if (options && options.errors) {
          options.errors.set(this, `Type '${this.rawType} for handle connection '${this.toPrettyString()}' doesn't match particle spec's type '${connectionSpec.type}'`);
        }
        return false;
      }
      if (this.direction != connectionSpec.direction) {
        if (options && options.errors) {
          options.errors.set(this, `Direction '${this.direction}' for handle connection '${this.toPrettyString()}' doesn't match particle spec's direction '${connectionSpec.direction}'`);
        }
        return false;
      }
    }
