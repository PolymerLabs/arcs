...
export class SlotDomRenderer extends SlotRenderer {
  constructor(slotContext, slotConsumer, containerKind) {
    super(slotContext, slotConsumer, containerKind);
  ...
  constructRenderRequest(slotConsumer) { 
    ...
    if (slotConsumer) { // eg hosted slot consumer
      prefixes.push(slotConsumer.consumeConn.particle.name);
      prefixes.push(slotConsumer.consumeConn.name);
    }
    if (!SlotDomRenderer.hasTemplate(prefixes.join('::'))) {
  ...
  createNewContainer(contextContainer, subId) {
    ...
    let consumeConn = this.slotConsumer.consumeConn;
    // Note: originally was set here to empty string, why?
    // can we reverse the order - particle.name::slotname?
    newContainer.setAttribute('particle-host', `${consumeConn.name}::${consumeConn.particle.name}`);
  ...
  setContainerContent(info, content, subId) {
    ...
    this._setTemplate(info, this.templatePrefix, content.templateName, content.template);
  ...
  dispose() {
    ...
    [...this._infoBySubId.values()].forEach(info => this.deleteContainer(info.container));
  }

  static render(container, content) {
    let renderer = new SlotDomRenderer();
    let info = {container, model: content.model};
    renderer._infoBySubId.set(undefined, info);
    renderer._eventHandler = () => {};
    renderer._stampTemplate(info, renderer.createTemplateElement(content.template));
    renderer._onUpdate(info);
    return renderer;
  }
  ...
  get templatePrefix() {
    return this._slotConsumer.consumeConn.prettyName;
  }

  _setTemplate(info, templatePrefix, templateName, template) {
    if (templateName) {
      info.templateName = [templatePrefix, templateName].filter(s => s).join('::');
      if (template) {
        if (templateByName.has(info.templateName)) {
          // TODO: check whether the new template is different from the one that was previously used.
          // Template is being replaced.
          this.clearContainer(info);
        }
        templateByName.set(info.templateName, this.createTemplateElement(template));
      }
    }
  }

  _onUpdate(info) {
    this._observe(info.container);

    if (info.templateName) {
      let template = templateByName.get(info.templateName);
      assert(template, `No template for ${info.templateName}`);
      this._stampTemplate(info, template);
    }

    this._updateModel(info, info.model);
  }
  ...
  _stampTemplate(info, template) {
    if (!info.liveDom) {
      // TODO(sjmiles): hack to allow subtree elements (e.g. x-list) to marshal events
      info.container._eventMapper = this._eventMapper.bind(this, this._eventHandler);
      info.liveDom = Template
          .stamp(template)
          .events(info.container._eventMapper)
          .appendTo(info.container);
    }
  }

  _updateModel(info, model) {
    let liveDom = info.liveDom;
    if (liveDom) {
      liveDom.set(model);
    }
  }
  ...
  initInnerContainers(container) {
    Array.from(container.querySelectorAll('[slotid]')).filter(innerContainer => {
      ...
      const providedSlotSpec = this.slotSpec.getProvidedSlotSpec(slotId);
      if (!providedSlotSpec) { // Skip non-declared slots
        console.warn(`Slot ${this.slotSpec.name} has unexpected inner slot ${slotId}`);
        return;
      }
  ...
  _initMutationObserver() {
    if (this.slotConsumer) {
      return new MutationObserver(async (records) => {
        ...
        let containers = [...this._infoBySubId.values()].map(info => info.container)
        ...
        if (containers.length > 0) {
          ...
          this.slotConsumer.updateProvidedContexts();
