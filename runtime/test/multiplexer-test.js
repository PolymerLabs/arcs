...
describe('Multiplexer', function() {
  ...
  it('renders polymorphic multiplexed slots', async function() {
    ...
    await helper.idle();
    assert.lengthOf(helper.slotComposer._slots.filter(s => s.constructor === Slot), 2);
    assert.lengthOf(helper.slotComposer._slots.filter(s => s.constructor === HostedSlotConsumer), 4);
    let itemSlot = helper.slotComposer._slots.find(s => s.consumeConn.name == 'item');
    assert(itemSlot);
    let items = [...itemSlot.renderer._infoBySubId.values()];
    ...
    SlotDomRenderer.hasTemplate('PostMuxer::item::ShowOne::item::default');
    SlotDomRenderer.hasTemplate('PostMuxer::item::ShowTwo::item::default');
    SlotDomRenderer.hasTemplate('PostMuxer::item::default');
    SlotDomRenderer.hasTemplate('Root::item::ShowOne::item::default');
  });
});
