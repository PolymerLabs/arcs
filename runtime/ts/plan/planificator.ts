...
export class Planificator {
  static async create(arc: Arc, {userid, protocol, onlyConsumer, devTools = false}) {
    ...
    const planificator = new Planificator(arc, userid, store, searchStore, onlyConsumer, devTools);
  ...
  constructor(arc: Arc, userid: string, store: StorageProviderBase, searchStore: StorageProviderBase, onlyConsumer: boolean, devTools: boolean) {
    this.arc = arc;
    this.userid = userid;
    this.searchStore = searchStore;
    if (!onlyConsumer) {
      this.producer = new PlanProducer(arc, store, searchStore, {devTools});
