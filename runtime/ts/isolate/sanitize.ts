...
let DYNAMIC_WHITELIST = new Set(CUSTOM_ATTRIBUTES);
...
export function sanitize(unsafeString: string): string {
     ...
     */
    let removedList = dompurify['removed'] as Array<Removed>;

    let attributesToWhiteList = removedList.filter((x => x.attribute && !/^on[^-]/i.test(x.attribute.name)))
