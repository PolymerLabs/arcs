...
export class ResultInspector {
   ...
   */
  constructor(arc: Arc, store: StorageProviderBase, field: string) {
...
export async function assertSingletonWillChangeTo(arc: Arc, store: StorageProviderBase, field: string, expectation) {
  await arc.idle;
  return assertSingletonIs(store, field, expectation);
}

export async function assertSingletonIs(store, field: string, expectation) {
