...
export interface Particle extends BaseToken {
  ...
  slots?: ParticleSlot[]; // TODO ParticleSlots, also not used in RecipeParticle
...
export type SchemaPrimitiveType = 'Text' | 'URL' | 'Number' | 'Boolean' | 'Bytes' | 'Object';
export type SchemaType = SchemaReferenceType | SchemaCollectionType | SchemaPrimitiveType | SchemaUnionType | SchemaTupleType;
...
export type ParticleArgumentType = VariableType | CollectionType | BigCollectionType | ReferenceType | SlotType | SchemaInline | TypeName;

// Note that ManifestStorage* are not here, as they do not have 'kind'
export type All = Import | 
  Meta | MetaName | MetaStorageKey |
  Particle | ParticleArgument | ParticleInterface |
  RecipeHandle | Resource |
  Shape | ShapeArgument | ShapeInterface | ShapeSlot;

export type ManifestItem = Recipe | Particle | Import | Schema | ManifestStorage | Shape | Meta | Resource;
