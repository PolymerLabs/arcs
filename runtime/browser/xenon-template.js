...
class Annotator {
  ...
  _annotateSubtree(node) {
    ...
    // return a locator only if there was at least one childLocator
...
let annotateEvent = function(node, key, notes, name, value) {
  if (name.slice(0, 3) === "on-") {
    if (value.slice(0, 2) === "{{") {
      ...
      console.warn(
        `Xenon: event handler for '{$name}' expressed as a mustache, which is not supported. Using literal value '${value}' instead.`
      );
    }
...
let stamp = function(template, opts) {
  ...
  // importing nodes can have side-effects, like CustomElement callbacks (before we
  // can do any work on the imported subtree).
  ...
  //postProcess(notes, map, controller, opts);
  // return dom manager
  return {
    root: root,
    notes: notes,
    map: map,
    mapEvents: mapEvents.bind(this, notes, map),
    set: set.bind(this, notes, map),
    appendTo: function(root) {
      if (this.root) {
        root.appendChild(this.root);
      } else {
        console.warn("template stamped no DOM");
      }
      this.root = root;
      return this;
    }
