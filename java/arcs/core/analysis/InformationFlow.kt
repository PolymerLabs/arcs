...
class InformationFlow private constructor(
  ...
  private val particleClaims = graph.particleNodes.associateBy(
    keySelector = { it.particle },
    valueTransform = {
      it.instantiatedClaims() +
        (typeVariableSemantics[it.particle]?.claims ?: emptyList()) +
        it.deduceClaims()
    }
  )
  ...
  private val labelIndices: Map<InformationFlowLabel, Int> = labels.mapIndexed { index, label ->
    label to index
  }.toMap()
